...
The portal account-creation blocker, [PR #190](https://github.com/markcuda/omarchy-kids-sandbox/pull/190), merged as `5571776`, identical to accepted candidate `41f4777`. The wizard now passes credentials appropriate to the trusted boot mode. Candidate formatter, Mac 46-file suite (five skips), VM 46-file suite (two skips), scenario 05 and independent review passed; the focused regression fails on previous main. Installed setup completed all five steps and created Ben with the selected settings. Real password login, launcher, GCompris menu, Finish authentication and parent return passed, with Quickshell watching verified after fresh logins. [Original screenshots](media/dogfood/portal-setup-41f4777/README.md) show the actual flow; no individual activity was played. Merged-main gates are running.

Setup still has separately filed misleading action/wording issues #188, #189 and #191, plus transient manifest errors #192 and existing Tux Paint availability #52. Ask, expiry, Wi-Fi and settings remain untested as complete current-candidate journeys. Removal is blocked in preflight by the fixture's retained disk-key mappings. The original VM snapshot and UEFI backup remain protected; final restoration is pending.
...
Merged main passed the formatter, Mac and VM suites, scenario 05, and the separate installed readback. The complete tree, package bytes, size and mtime, and installed pins remain unchanged, so the four explicitly premerge UI routes remain valid without a duplicate walkthrough. The VM returned to its original Latte greeter. The Air’s current Vantablack theme was preserved after an obsolete Tokyo Night check failed; no cause for that external change is asserted.

The Advanced checklist clipping fix, [PR #173](https://github.com/markcuda/omarchy-kids-sandbox/pull/173), merged as `6fa6d30`, with the complete tree identical to candidate `9e9200aa`. Every selected app now appears in the card, while the setting row shows a compact count. The candidate passed formatting, 44 Mac test files (five skips), 44 VM test files (two skips), and scenario 05. Four installed previews in fresh watched Latte and Tokyo Night sessions verified eight apps, editing to seven with matching Ready summaries, and all sixteen apps plus controls visible for the largest pack. Independent image review and a separate restoration reader passed; all 181 package files were intact, original settings/themes/greeter restored, and all owned processes and Ben account/profile absent. [Eleven original frames](media/dogfood/wizard172-9e9200/README.md) document the result. No Apply was used.

Merged main passed formatting, the same 44-file Mac and VM suites with their reported skips, and scenario 05, followed by a separate installed readback. All 181 package files, runtime pins, timestamp, original themes/settings/timer/greeter and config cleanup passed. Its identical full tree and unchanged package SHA, size and mtime preserve the four explicitly premerge UI routes; no duplicate postmerge walkthrough is claimed. [#184](https://github.com/markcuda/omarchy-kids-sandbox/issues/184) records a minor Leave-confirmation border scroll for the sixteen-app screen in both themes; every label and control remains visible. The redundant confirmation question remains [#182](https://github.com/markcuda/omarchy-kids-sandbox/issues/182), with source-approved draft PR #183 awaiting gates. Lower-impact findings stay in tickets.

[PR #186](https://github.com/markcuda/omarchy-kids-sandbox/pull/186), for #185, merged as `d291db1`, with the complete tree identical to accepted candidate `11383c8`. Real setup had twice broken after choosing an age: background sudo restored cooked terminal flags while the same Gum chooser remained open. Prefetch now receives stdin from `/dev/null`. The candidate passed formatting, 45 Mac files (five skipped checks), 45 VM files (two skipped checks), and scenario 05; the new owned-PTY regression fails against the prior wizard. The installed authenticated Ben/Fox/Ages 6–8 route then stayed clean through Mode and Enter to Web. Ctrl+C/Yes closed setup before Apply. A read-only observer confirmed the actual prefetch commands and stdin isolation, with the same Gum retaining raw flags after prefetch disappeared; prefetch exit status was not observed. Independent review and separate package/settings/greeter readback passed. Ben and all owned processes remained absent. [Before/after screenshots](media/dogfood/wizard-prefetch-main-6fa6d30/README.md) document this scoped result. Merged main passed the same ordered formatter, Mac and VM suites, scenario 05, and a fresh installed readback. The complete tree and artifact SHA, size, and mtime matched, so the independently approved premerge UI evidence remains valid; no new postmerge UI run is claimed.

Full portal provisioning was attempted with the invented Ben/Fox/Ages 6–8 fixture and failed at account creation. [#187](https://github.com/markcuda/omarchy-kids-sandbox/issues/187) records the wizard passing a disk-only parent-password option; its narrow repair is selected. [#188](https://github.com/markcuda/omarchy-kids-sandbox/issues/188) records misleading readiness/actions after the failure, and [#189](https://github.com/markcuda/omarchy-kids-sandbox/issues/189) records the portal password screen promising disk unlock. [Two original screenshots](media/dogfood/portal-setup-d291db1/README.md) show the distinct failures. Ben remained absent; the owned setup/watch processes exited, the greeter returned, and separate package/settings readback passed. Kid login, launcher/apps, exit/return, Ask/approval, time expiry, Wi-Fi, and parent settings remain untested as complete journeys on this candidate. Full-removal preflight is blocked by the disposable portal fixture's retained disk-key mappings; no removal or boot-mode bypass was attempted. The original offline disk snapshot and separate UEFI backup remain protected for final restoration. #98 still requires Mark’s #109 decision; #97 waits for #98, then #99.
...
Merged main passed the formatter, Mac suite (42 files, five skips), VM suite (42 files, two skips), and the installed repeat in both fresh watched themes. Independent screenshot review and a separate restoration readback passed. All 181 package files remain intact, original settings/themes/timer are restored, only the greeter remains, and no Ben account was created. [Twelve new merged-main screenshots](media/dogfood/wizard167-main-b553913/README.md) document summary/default/custom/Back/Leave; 55 originals are retained. One Mac provisioning check returned 141 in the parallel pass and passed its serial retry; the cause remains unverified. Local storage failures were recovered using verified external-cache copies; no acceptance screenshot was lost.

[PR #171](https://github.com/markcuda/omarchy-kids-sandbox/pull/171), for hidden weekend time limits, passed the test-box formatter, the 42-file Mac suite with five skips, the same VM suite with two skips, and scenario 05. Its installed candidate passed default/custom weekend values and Back-to-Time acceptance in both fresh watched themes, independent screenshot review, and separate restoration readback. All 181 package files remain intact; original settings/themes/timer/greeter are restored and Ben is absent. [Sixteen exact screenshots](media/dogfood/wizard170-598a82f/README.md) document the candidate; 81 originals are retained. It merged as `069c584` with the identical tested tree. Merged main passed the formatter, Mac 42/five skips, VM 42/two skips, scenario 05, and the installed full route in both fresh watched themes. Independent screenshot review and a separate restoration readback passed; all 181 package files are intact, original settings/themes/timer/greeter restored, Ben absent. [Sixteen new merged-main screenshots](media/dogfood/wizard170-main-069c584/README.md) are published; 77 originals are retained. A separate [blank Advanced editor finding](media/dogfood/wizard-current-input-598a82f/README.md) is tracked in [#176](https://github.com/markcuda/omarchy-kids-sandbox/issues/176); reviewed [draft PR #177](https://github.com/markcuda/omarchy-kids-sandbox/pull/177) awaits full gates and installed proof after #175. New [#172](https://github.com/markcuda/omarchy-kids-sandbox/issues/172) records the Advanced app list clipping in both themes; reviewed draft [PR #173](https://github.com/markcuda/omarchy-kids-sandbox/pull/173) exposes the full list in the card and is now rebased onto merged main without code changes. Its full gates and live rendering are pending. Reviewed [draft PR #175](https://github.com/markcuda/omarchy-kids-sandbox/pull/175) keeps desktop choices in plain words across the wizard; it is stacked behind #173 and awaits full gates/live proof. The [QMP colon input diagnostic](media/dogfood/qmp-colon-598a82f/README.md) is retained separately; it is tracked in [#178](https://github.com/markcuda/omarchy-kids-sandbox/issues/178), with source-reviewed [draft PR #179](https://github.com/markcuda/omarchy-kids-sandbox/pull/179) awaiting full gates and named live proof. The #98/#109 human decision, #97 ordering hold, and three full release walkthroughs remain open.
