...
`portal` to `disk` validates the LUKS root and all required secrets before changing boot artifacts. It securely prompts for the current disk passphrase and, in deterministic account order, each existing passworded kid's current password. It adds and verifies every missing kid slot, writes the slot map, installs the mkinitcpio drop-in, rebuilds once, verifies the UKI, updates Limine where present, and writes `boot=disk` last. A failure keeps `boot=portal`, rolls back slots added by that attempt, and does not report success.

`disk` to `portal` writes `boot=portal` first so concurrent assert and boot-login become no-ops for disk behavior. It kills every recorded kid slot, removes the slot map and mkinitcpio drop-in, restores only Kids Mode-owned Limine state, runs exactly one `mkinitcpio -P`, and verifies that the hook is absent from the current UKI. Failure returns nonzero and leaves portal as the authoritative safe mode; rerunning the same command resumes convergence.
