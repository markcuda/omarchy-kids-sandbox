...
| `scripts/vm-qmp.sh` | `shot out.png` screenshots the console; `printf '%s' "$DISK_PASSWORD" | scripts/vm-qmp.sh type` and `enter` type at the disk prompt; `quit` powers off |
...
prompt until the disk password is piped to `vm-qmp.sh type` and `vm-qmp.sh enter` is sent. The
`type` command reads only stdin; it does not accept the password as an argument.
...
The Mac-side boot drivers read their password from stdin so it never enters their process argv:
`printf '%s\n' "$DISK_PASSWORD" | SSH_CFG="$CFG" scripts/v6-limine.sh "$SHOTS_DIR"` and
`printf '%s\n' "$SECOND_PASSWORD" | SSH_CFG="$CFG" scripts/v1-two-sessions.sh "$SECOND_USER" "$SHOTS_DIR"`.
