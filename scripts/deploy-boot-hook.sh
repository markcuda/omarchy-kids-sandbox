...
if [[ ${1:-} == --remove ]]; then
  ...
  systemctl daemon-reload; mkinitcpio -P 2>&1 | grep -E "Image generation|WARNING|ERROR" | head -6; echo "removed"; exit 0
fi
...
install -d -m 700 /etc/omarchy-kids; touch /etc/omarchy-kids/luks-slots; chmod 600 /etc/omarchy-kids/luks-slots
systemctl daemon-reload; systemctl enable omarchy-kids-boot-login.service omarchy-kids-boot-login-cleanup.service >/dev/null 2>&1
...
if [[ -n $UKI ]]; then
  objcopy -O binary --only-section=.initrd "$UKI" /tmp/ir.img && echo "hook files in the boot image: $(lsinitcpio /tmp/ir.img | grep -c 'omarchy-kids')"; rm -f /tmp/ir.img
fi
