...
if ! sudo test -f "$KEY"; then sudo bash -c "head -c 64 /dev/urandom > $KEY"; sudo chmod 600 "$KEY"; fi
if sudo cryptsetup open --test-passphrase --key-file "$KEY" "$DEV" 2>/dev/null; then
  ...
  ok=0
  for i in 1 2 3; do
    if sudo cryptsetup luksAddKey "$DEV" "$KEY"; then ok=1; break; fi
    echo "That password did not open the disk. Try again ($i/3)."
  done
  [[ $ok == 1 ]] || { echo "Could not add the key. Nothing else was changed."; exit 1; }
