...
set -uo pipefail
CFG="${SSH_CFG:?set SSH_CFG}"; U2="${1:?user}"; P2="${2:?password}"; OUT="${3:-/tmp}"
vm(){ ssh -F "$CFG" vm "$@"; }; air(){ ssh -F "$CFG" air "$@"; }
...
echo "== sessions before =="; vm 'loginctl list-sessions --no-legend'
first=$(vm 'loginctl list-sessions --no-legend | awk "\$5==\"user\" && \$7!=\"-\" {print \$3; exit}"'); echo "first graphical user: ${first:-?}"
...
sleep 12; shot v1-greeter
echo "== typing $U2 on the greeter =="
qmp type "$U2"; qmp key tab; qmp type "$P2"; qmp enter
sleep 40; shot v1-second-session
echo "== sessions after =="; vm 'loginctl list-sessions --no-legend; for s in $(loginctl list-sessions --no-legend | awk "{print \$1}"); do echo "session $s: $(loginctl show-session $s -p Name -p VTNr -p Active -p State --value | tr "\n" " ")"; done'
echo "== VT exposure: Ctrl+Alt+F1 then F2, screenshots =="
qmp key ctrl-alt-f1; sleep 4; shot v1-vt1; qmp key ctrl-alt-f2; sleep 4; shot v1-vt2
echo "== done; first user's session state =="; vm "loginctl list-sessions --no-legend | grep ' $first ' || true"
