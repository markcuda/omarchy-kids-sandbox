...
set -euo pipefail
VM="${VM_DIR:-$HOME/vm}"
ISO="$VM/omarchy-4.0.2.iso"
USER_NAME="${VM_USER:-kid-vm}"
PASS="${VM_PASS:-omarchy}"
HOST="${VM_HOST:-kids-vm}"
TZ_NAME="${VM_TZ:-America/New_York}"
ENCRYPT="${VM_ENCRYPT:-true}"
SIZE_GB="${VM_SIZE_GB:-40}"
...
_EOF_
echo "Kid VM" >cidata/user_full_name.txt
echo "kids@example.test" >cidata/user_email_address.txt
...
mkfs.fat -n cidata cidata.img >/dev/null
