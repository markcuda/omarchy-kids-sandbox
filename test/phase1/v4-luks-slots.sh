...
slots(){ sudo cryptsetup luksDump "$DEV" | grep -cE '^\s+[0-9]+: luks2'; }
echo "device: $DEV  slots before: $(slots)"
add(){ sudo cryptsetup luksAddKey --key-file=<(printf '%s' "$OWNER_PASS") "$DEV" <(printf '%s' "$1") 2>&1; }
test_pw(){ sudo cryptsetup open --test-passphrase --key-file=<(printf '%s' "$1") "$DEV" 2>/dev/null; }
...
sudo cryptsetup luksAddKey --key-file=<(printf '%s' kidpass-ben) "$DEV" <(printf '%s' kidpass-ben2) 2>/dev/null \
 && sudo cryptsetup luksRemoveKey "$DEV" --key-file=<(printf '%s' kidpass-ben) 2>/dev/null \
...
# remove a kid
sudo cryptsetup luksRemoveKey "$DEV" --key-file=<(printf '%s' kidpass-cy) 2>/dev/null && ! test_pw kidpass-cy && pass "kid removed, their password no longer opens the disk" || fail "remove kid"
...
sudo cryptsetup luksDump "$DEV" | grep -E '^\s+[0-9]+: luks2'
