...
set -uo pipefail
R="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"; PARENT=kid-test; PPASS="v5-parent-pass"; KID=kid-ada; KPASS="v5-kid-pass"
rc=0; pass(){ echo "PASS  $*"; }; fail(){ echo "FAIL  $*"; rc=1; }
id $KID >/dev/null 2>&1 || sudo useradd -m -G omarchy-kids -s /bin/bash $KID
printf '%s:%s\n' $PARENT "$PPASS" | sudo chpasswd; printf '%s:%s\n' $KID "$KPASS" | sudo chpasswd
...
r=$(try $KID $KID "$KPASS");   [[ $r == PAM_SUCCESS ]] && pass "kid's own password opens the kid's stack (pam_unix)" || fail "kid own password: $r"
r=$(try $KID $KID "$PPASS");   [[ $r == PAM_SUCCESS ]] && pass "parent password opens the kid's stack (verifier), stack run as the kid" || fail "parent pw as kid: $r"
r=$(try root $KID "$PPASS");   [[ $r == PAM_SUCCESS ]] && pass "parent password opens the kid's stack, stack run as root (SDDM case)" || fail "parent pw as root: $r"
r=$(try root $KID "nope-1"); r2=$(try root $KID "nope-2"); r3=$(try root $KID "nope-3"); [[ $r3 != PAM_SUCCESS ]] && pass "wrong password denied ($r3)" || fail "wrong accepted"
r=$(try $KID $KID "$PPASS");   [[ $r != PAM_SUCCESS ]] && pass "after 3 misses the parent path is rate-limited (30 s)" || fail "rate limit missing"
r=$(try $KID $KID "$KPASS");   [[ $r == PAM_SUCCESS ]] && pass "kid's own password still works while the parent path is limited" || fail "kid pw during limit: $r"
...
echo "V5 RESULT: $([[ $rc == 0 ]] && echo PASS || echo FAIL)"; exit $rc
