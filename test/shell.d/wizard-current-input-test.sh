...
BAND=6-8 BUDGET_MIN_WEEKEND=60 LIGHTS_OUT_WEEKEND=20:00
...
grep -qx -- '--value' "$RENDER_LOG" && grep -qx -- '60' "$RENDER_LOG" || {
...
grep -qx -- '--value' "$RENDER_LOG" && grep -qx -- '20:00' "$RENDER_LOG" || {
